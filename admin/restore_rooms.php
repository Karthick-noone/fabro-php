...
// Function to update available rooms and restore after length_of_stay completes
function updateAvailableRooms($data, $conn)
{
    ...
    foreach ($data['roomTypes'] as $index => $roomType) {
        ...
        // Restore availability after length_of_stay completes
        // Assuming that $data['lengthOfStay'] contains the completed length_of_stay
        if ($lengthOfStay <= 0) {
            $stmtRestore = $conn->prepare("UPDATE rooms SET currently_available = currently_available + ?, length_of_stay = 0 WHERE room_type = ?");
            $stmtRestore->execute([$data['numberOfRooms'][$index], $roomType]);
        } else {
            $stmtRestore = $conn->prepare("UPDATE rooms SET length_of_stay = length_of_stay - 1 WHERE room_type = ?");
            $stmtRestore->execute([$roomType]);
        }
    }
