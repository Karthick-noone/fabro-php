...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['new_image']) && $_FILES['new_image']['error'] == 0) {
        ...
        $imageFolder = 'images/slider_images/'; // Adjust the folder path as needed
        $newImagePath = '../' . $imageFolder . $imageName;
        move_uploaded_file($_FILES['new_image']['tmp_name'], $newImagePath);

        // Update the image path in the database
        $sql = "UPDATE picture SET bg_photo = :newImagePath, imagepath = :imagepathh WHERE id = :id";
        $parameters = [
    ...
    if ($stmt->execute($parameters)) {
        ...
        header("Location: image.php"); // Redirect to gallery.php
        exit();
    } else {
